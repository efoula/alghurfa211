<?php
$cur_issue_id = $_POST['issue_id'];
$issue_name = get_term( $cur_issue_id )->name;
$issue_desc = get_term( $cur_issue_id )->description;
$get_issue_posttypes = get_posttypes_of_issue($cur_issue_id);
?>

<!-- Section 1 Starter -->
<section class="pt-8 mb-10 lg:pt-20 lg:mb-24">
    <div
        class="flex items-center flex-col lg:flex-row carousel-item justify-center lg:gap-[60px] gap-6">
        <div>
        ...
        <div>
            ...
            <div class="flex-wrap font-semibold font-cairo gap-4 max-w-[600px] hidden lg:flex">
                <a href="#intro"
                    class="py-1 rounded-lg shadow-md text-main bg-light-gray px-7 text-dark">مقدمة</a>
                <?php
                if($get_issue_posttypes) {
                    foreach ($get_issue_posttypes as $issue_post_type) {
                ?>
                    <a href="#<?php echo$issue_post_type;?>"
                    class="py-1 rounded-lg shadow-md text-main bg-light-gray px-7 text-dark"><?php echo get_translation($issue_post_type);?></a>
                <?php } } ?>
            </div>
        </div>
    </div>
...
<!-- Section 2 Starter --> 
<section class="flex flex-col md:flex-row items-center md:gap-[50px] gap-8 md:mb-20 mb-8" id="intro">
    <h3 class="text-2xl font-extrabold text-center md:text-[25px] md:text-start font-notoSans text-dark">
...
if($get_issue_posttypes) {
    foreach ($get_issue_posttypes as $issue_post_type) {
        $p_layout = get_term_meta($cur_issue_id, 'dlayout_'.$issue_post_type.'', true);
        echo '<section class="flex flex-col xl:flex-row items-center xl:gap-[50px] gap-8 xl:mb-20 mb-8" id="'.$issue_post_type.'">
            <h3 class=" text-2xl font-extrabold text-center md:text-[25px] md:text-start font-notoSans
            text-dark">'.get_long_titles($issue_post_type).'</h3>';


        if($p_layout == 'layout_1' || $p_layout == '') { 
            $perpage = 4; 
        ...
        $query = new WP_Query(array(
            ...
            'tax_query'      => array(
                array(
                    'taxonomy' => 'issues',
                    'terms'    => $cur_issue_id, // grouped category
                ),
            )
        )); 
        $count = 0; 
...
?>
    ...
    <?php if($p_layout == 'layout_1' || $p_layout == '') { ?>
        ...
        <!-- Section 3 Starter layout_1 -->
            <a href="<?php echo $p_permalink; ?>" class="group">
                ...
                <div
                    class="group-hover:bg-black bg-[#F3F3F3] h-[10px] w-[249px] rounded-[10px] mt-8 transition-all ease-in-out duration-500 hidden lg:block">
                </div>
            </a>
        <!-- Section 3 End -->
    ...
    <?php } elseif($p_layout == 'layout_4') { ?>
        ...
        <!-- Section 6 Starter layout_4 -->
            <div class="relative flex-1 overflow-hidden">
                ...
                <p class="mt-4 text-base font-normal text-justify text-black lg:mt-6 lg:font-light font-droid lg:text-2xl lg:leading-loose"><?php echo $p_excerpt; ?></p>
            </div>
        <!-- Section 6 End -->
    ...
    <?php } elseif($p_layout == 'layout_5') { ?>
        ...
        <!-- Section 7 Starter layout_5 -->
            ...
            <a href="<?php echo $p_permalink; ?>" class="lg:w-[330px] w-[180px] h-[180px] lg:h-[330px] rounded-full border-[10px] border-primary bg-primary hover:bg-white transition flex items-center justify-center px-6 font-extrabold lg:text-2xl text-xl text-black text-center"><?php echo $p_nme; ?></a>
